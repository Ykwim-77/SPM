  ...
  return (
    <SafeAreaView style={styles.wrap} edges={['top']}>
      <View style={styles.header}>
        <AccessiblePressable onPress={() => router.back()} style={styles.back} testID="exams-back" label="Voltar">
          <Ionicons name="arrow-back" size={24} color={colors.onSurface} />
        </AccessiblePressable>
        <Text style={styles.title}>Exames e Documentos</Text>
        <View style={{ width: 44 }} />
      </View>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <AccessiblePressable style={styles.card} onPress={() => router.push(`/exams/${item.id}`)} testID={`exam-card-${item.id}`} label={`Exame: ${item.title}`}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.cardMeta}>{item.doctor_name || 'Documento'} • {dayjs(item.date).format('DD/MM/YYYY')}</Text>
              </View>
